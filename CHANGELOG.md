...
> 「服务版本」一节会显示服务代码是否最新;有新提交时点「更新服务」即可。
> 内存 16 GB 以上的 Mac 更新后 LLM 会换成 Gemma-4-E4B,首次加载要下载约 6.8 GB。
> 用的是自己改过的提示词的,要重新套一次预设(「设置 → 服务 → 提示词 → 套用预设」)
> 才有格式整理。

### Added

- **随时能检查服务有没有更新**:以前「服务版本」一节和「更新服务」按钮只在服务和应用的
  版本号对不上时才出现,可版本号只在发版时变,两次发版之间合进来的服务端修复看不出来,
  也没法主动更新。现在本地模式下这一节一直显示:服务代码在哪个提交、是否最新,「检查
  服务更新」(只 fetch,不改仓库)列出新提交;「更新服务」一直可用。启动 15 秒后和打开
  「服务」页时会自动查一次,有新提交时主界面提示。

### Changed

- **LLM 后处理会整理格式了(对标 Typeless)**:口述里列举几件事、几点意见、一串东西、
  操作步骤时整理成编号列表;很长又换了话题(「另外」「还有一件事」)时分段;连环改口
  (「周二不对周三下午三点哦不对四点」)只留最后的说法;金额、百分比、电话号码写成数字。
  短句、只是提到「第一个方案」的句子照旧一行话,不会被硬拆成列表。默认提示词和三个预设
  (中英两份)都换成同一套规则。
- **内存 16 GB 以上的机器默认 LLM 换成 Gemma-4-E4B**(QAT 4bit,约 6.4 GB):新的 18 例
  格式整理检查里它 18/18;E2B 16/18,其中一处改了意思(把「两斤苹果」挂到了鸡蛋上)。
  内存不到 16 GB 的机器仍默认 E2B。E4B 短句约 1 秒、400 字的长口述约 6 秒(E2B 约 3 秒)。
...
- **边录边传、录音期间分段识别**:按下快捷键就连上服务、边录边传,服务端在录音期间
  每攒够 28 秒,就在 18–28 秒之间最安静的地方切一段先识别;松手时只剩最后一小段。实测(M3 Max,
