...
| H3 | **async 函数内嵌 `asyncio.run`** | `services/stt_server.py:358`(加载)与 `:511`(whisper_cpp transcribe)在事件循环内调用 `asyncio.run(...)`,运行时报 `RuntimeError: asyncio.run() cannot be called from a running event loop` |
...
| M2 | **STT 服务职责混杂:整组转发 `/llm/*` 端点** | `services/stt_server.py:719-819` 用 httpx 转发;`client/app.py:41` 却直连 6545,转发层实际没被客户端使用 |
...
| L2 | `stt_server.py:534-552` 大部分为仅 qwen-native 可达的死代码路径;单文件 40KB(FastAPI app 640 行 + STTEngine 236 行 + WS 端点)过大,建议拆分模块 |
| L3 | CI lint 不阻塞:`|| true` | `.github/workflows/ci.yml:51-52,73-77` |
| L4 | `pyproject.toml` 版本 2.0.1 与 CHANGELOG 中 v2.0.10 不一致;`[project.scripts]` 的 `voice-server` 指向 `services.stt_server:main` 而 README 用 `python -m services.stt_server`,需对齐 |
...
3. **消除 `asyncio.run` 嵌套**
   - 将 358 行与 511 行的同步调用改为 `await loop.run_in_executor(None, ...)`(与主模型加载方式 `stt_server.py:272` 一致),或让 whisper.cpp 引擎暴露 async 接口。
...
6. **移除 STT 服务的 LLM 转发层**,客户端统一直连 6545;或反过来由 STT 服务统一代理、客户端只连 6544——二选一,避免双路径。
...
项目架构方向正确(双服务分离解决依赖冲突),`services/` 层是稳定核心;主要风险集中在**迁移未完成导致的死代码与失效入口**、**STT 引擎多分支返回类型不一致**、**伪流式与文档不符**三处。建议按 4.1 → 4.2 → 4.3 顺序推进,先修 H1–H4 四个必然出错点,再做结构清理。
