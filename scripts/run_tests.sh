...
/opt/homebrew/Caskroom/miniconda/base/bin/conda run -n vif-stt -- python -m pytest \
    "$PROJECT_DIR/tests/test_stt_server.py" \
    -v --tb=short -m "not integration"

echo ""
echo "=== Testing LLM Server ==="
/opt/homebrew/Caskroom/miniconda/base/bin/conda run -n mlx-test -- python -m pytest \
