...
class _MacOSEventTapListener:
    ...
    # macOS HID 键码 → 字符(ANSI 布局)。注意:CGEventTap 给的键码是
    # macOS HID 键码,字母 A=0x00、数字 1=0x12 —— 不是 Windows VK!
    _MAC_HID_TO_CHAR = {
        0x00: "a",
        0x01: "s",
        0x02: "d",
    ...
    def _key_from_vk(self, vk: int) -> _TapKey:
        """macOS HID 键码 → _TapKey(带 pynput 风格名称)

        注意:CGEventTap 给的键码是 macOS HID 键码(字母 A=0x00、数字
        1=0x12),**不是** Windows 虚拟键码(0x41-0x5A)。修饰键/功能键查
        _KEY_NAMES,字母/数字查 _MAC_HID_TO_CHAR。
        """
        name = self._KEY_NAMES.get(vk)
        if name:
            return _TapKey(name, vk)
        # 字母/数字键(macOS HID 键码 → 字符)
        c = self._MAC_HID_TO_CHAR.get(vk)
        if c:
            return _TapKey(c, vk, char=c)
        # 其他特殊键
