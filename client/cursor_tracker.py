...
# ============================================================================
    ...
    _AXUIElementCopyAttributeValue.argtypes = [
        AXUIElement,
        ctypes.c_void_p,
    ...
    _AXUIElementCopyParameterizedAttributeValue.argtypes = [
        AXUIElement,
        ctypes.c_void_p,
        CFTypeRef,
        ctypes.POINTER(CFTypeRef),
    ...
    def get_text_cursor_position() -> Optional[Tuple[int, int]]:
        ...
        try:
            ...
            error = _AXUIElementCopyAttributeValue(
                system_wide, kAXFocusedUIElementAttribute, ctypes.byref(focused_element)
            )
            if error != 0:
                return None
            ...
            error = _AXUIElementCopyAttributeValue(
                focused_element, kAXSelectedTextRangeAttribute, ctypes.byref(text_range)
            )
            if error != 0:
                # 尝试获取整个元素的边界
            ...
            if _AXValueGetValue(bounds, kAXValueCGRectType, ctypes.byref(rect)):
                ...
                if screen_height:
                    ...
                    return (
                        int(rect.origin.x),
                        int(screen_height - rect.origin.y - rect.size.height),
                    )
    ...
    def get_focused_window_title() -> str:
        ...
        try:
            ...
            title = CFTypeRef()
            error = _AXUIElementCopyAttributeValue(
                focused_element, kAXTitleAttribute, ctypes.byref(title)
            )
            if error == 0:
                # title 是 CFString，需要转换
                try:
                    ctypes.cast(title, ctypes.c_void_p)
                    # 简单的 CFString 提取
                    return "Window"  # 简化处理
                except Exception:
                    pass
